...
func (m PanelBarChartDataSourceModel_Options) MarshalJSON() ([]byte, error) {
	...
	var attr_colorbyfield *string
	if !m.ColorByField.IsNull() && !m.ColorByField.IsUnknown() {
		tmp := m.ColorByField.ValueString()
		attr_colorbyfield = &tmp
	}
	attr_orientation := m.Orientation.ValueString()
	var attr_barradius *float64
	if !m.BarRadius.IsNull() && !m.BarRadius.IsUnknown() {
		tmp := m.BarRadius.ValueFloat64()
		attr_barradius = &tmp
	}
	attr_xticklabelrotation := m.XTickLabelRotation.ValueInt64()
	attr_xticklabelmaxlength := m.XTickLabelMaxLength.ValueInt64()
	var attr_xticklabelspacing *int64
	if !m.XTickLabelSpacing.IsNull() && !m.XTickLabelSpacing.IsUnknown() {
		tmp := m.XTickLabelSpacing.ValueInt64()
		attr_xticklabelspacing = &tmp
	}
