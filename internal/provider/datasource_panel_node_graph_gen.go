...
func (m PanelNodeGraphDataSourceModel_Options_Nodes) MarshalJSON() ([]byte, error) {
	...
	m = m.ApplyDefaults()
	var attr_mainstatunit *string
	if !m.MainStatUnit.IsNull() && !m.MainStatUnit.IsUnknown() {
		tmp := m.MainStatUnit.ValueString()
		attr_mainstatunit = &tmp
	}
	var attr_secondarystatunit *string
	if !m.SecondaryStatUnit.IsNull() && !m.SecondaryStatUnit.IsUnknown() {
		tmp := m.SecondaryStatUnit.ValueString()
		attr_secondarystatunit = &tmp
	}
	attr_arcs := []interface{}{}
