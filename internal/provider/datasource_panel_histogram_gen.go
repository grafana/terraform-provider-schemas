...
func (m PanelHistogramDataSourceModel_Options) MarshalJSON() ([]byte, error) {
	...
	m = m.ApplyDefaults()
	var attr_bucketsize *int64
	if !m.BucketSize.IsNull() && !m.BucketSize.IsUnknown() {
		tmp := m.BucketSize.ValueInt64()
		attr_bucketsize = &tmp
	}
	var attr_bucketoffset *int64
	if !m.BucketOffset.IsNull() && !m.BucketOffset.IsUnknown() {
		tmp := m.BucketOffset.ValueInt64()
		attr_bucketoffset = &tmp
	}
	...
	var attr_combine *bool
	if !m.Combine.IsNull() && !m.Combine.IsUnknown() {
		tmp := m.Combine.ValueBool()
		attr_combine = &tmp
	}
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"type": schema.StringAttribute{
				MarkdownDescription: `The panel plugin type id. May not be empty. Defaults to "histogram".`,
				Computed:            true,
				Optional:            true,
				Required:            false,
			},
